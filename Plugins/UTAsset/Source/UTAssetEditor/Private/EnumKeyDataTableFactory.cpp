...
#define LOCTEXT_NAMESPACE "EnumKeyDataTableFactory"

UClass* FAssetTypeActions_EnumKeyDataTable::GetSupportedClass() const
{
	return UEnumKeyDataTable::StaticClass();
}

UEnumKeyDataTableFactory::UEnumKeyDataTableFactory(const FObjectInitializer& ObjectInitializer)
	:Super(ObjectInitializer)
{
	bCreateNew = true;

	SupportedClass = UEnumKeyDataTable::StaticClass();
	bEditAfterNew = 1;
}

bool UEnumKeyDataTableFactory::ConfigureProperties()
{
	class FEnumKeyDataTableStructFilter : public IStructViewerFilter
	{
	public:
	...
	public:
		...
		TSharedRef<SWidget> GenerateStructPicker()
		{
			...
			Options.StructFilter = MakeShared<FEnumKeyDataTableStructFilter>();
		...
		const UScriptStruct* OpenStructSelector()
		{
			...
			PickerWindow = SNew(SWindow)
				.Title(LOCTEXT("EnumKeyDataTableFactoryOptions", "Pick Row Structure"))
...
UObject* UEnumKeyDataTableFactory::FactoryCreateNew(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	return NewObject<UEnumKeyDataTable>(InParent, InClass, InName, Flags);
}

bool UEnumKeyDataTableFactory::ShouldShowInNewMenu() const
{
	return true;
}

UDataTable* UEnumKeyDataTableFactory::MakeNewDataTable(UObject* InParent, FName Name, EObjectFlags Flags)
{
	return NewObject< UEnumKeyDataTable>(InParent, Name, Flags);
}
